...
const unfundedGamesCount = unfundedGames.reduce((acc, game) => {
  return acc + 1;
}, 0);
...
const displayStr = `
    A total of $${totalRaised.toLocaleString(
      "en-US"
    )} has been raised for ${totalGames.toLocaleString("en-US")} game${
  totalGames > 1 ? "s" : ""
...
let firstGameDisplay = document.createElement("p");
firstGameDisplay.innerHTML = `<p>${firstGame.name}</p>`;
firstGameContainer.appendChild(firstGameDisplay);

// do the same for the runner up item
let secondGameDisplay = document.createElement("p");
secondGameDisplay.innerHTML = `<p>${secondGame.name}</p>`;
secondGameContainer.appendChild(secondGameDisplay);

// Customizations: search function implementation
...
// function to search games matching user input
